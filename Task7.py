...
from pyspark.sql.functions import col, month, avg, dayofmonth, concat, lit
...
if __name__ == "__main__":
    ...
    # task 7
    # ref: code from task 3-3
    route_df = df.withColumn("Route", concat(col("Pickup_Borough"),lit(" to "),col("Dropoff_Borough")))
    
    
    route_counts = route_df.groupBy('Route') \
    .pivot('business') \
    .count() \
    .fillna(0) \
    .withColumn('total_count', col('Lyft') + col('Uber')) \
    .withColumnRenamed('Lyft', 'lfty_count') \
    .withColumnRenamed('Uber', 'uber_count')

    # Select the 'Route' and 'total_count' columns
    top_routes = route_counts.select('Route','uber_count','lfty_count','total_count')

    # Sort the result by 'total_count' in descending order
    top_routes = top_routes.orderBy(col('total_count').desc()).limit(10)
