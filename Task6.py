...
if __name__ == "__main__":
    ...
    # task 6-1
    #count_df = df.groupBy("Pickup_Borough", "time_of_day").count() 
    #new_dataframe = count_df.select("Pickup_Borough","time_of_day", "count").filter(col('count') <= 1000)
    #new_dataframe = new_dataframe.withColumnRenamed("count", "trip_count")

    # task 6-2
    #count_df = df.groupBy("Pickup_Borough", "time_of_day").count() 
    #new_dataframe = count_df.select("Pickup_Borough","time_of_day", "count").filter(col('time_of_day') == "evening")

    # task 6-3
    filtered_df = df.filter((col('Pickup_Borough') == "Brooklyn") &(col('Dropoff_Borough') == "Staten Island"))
    new_dataframe = filtered_df.select("Pickup_Borough","Dropoff_Borough", "Pickup_Zone")
    # Count the number of trips
    num_trips = filtered_df.count()
    
    print("======== count=======")
    print("Number of trips starting in Brooklyn and ending in Staten Island:", num_trips)
    print("======== count=======")
    # Show the result
    #new_dataframe = new_dataframe.withColumnRenamed("count", "trip_count")
    ...
    #print("=========== task 2-3 ==================")
    new_dataframe.show(10, truncate = False)
    ###############################################
