...
if __name__ == "__main__":
    ...
    df = df.withColumn('business-month', concat(col("business"),lit("-"),col("Month")))

    # task 2-1
    #count_df = df.groupBy("business-month").count()

    #new_dataframe = count_df.select('business-month', 'count')

    #new_dataframe.show(new_dataframe.count())
    
    # task 2-2
    # Group by business and month, and calculate the total profit
    
    #platform_profits = df.groupBy("business-month") \
    #                .agg({"rideshare_profit": "sum"}) \
    #                .withColumnRenamed("sum(rideshare_profit)", "total_profit")


    #new_dataframe = platform_profits.select('business-month', 'total_profit')

    # task 2-3
    driver_profits = df.groupBy("business-month") \
                    .agg({"driver_total_pay": "sum"}) \
                    .withColumnRenamed("sum(driver_total_pay)", "driver_total_pay")


    new_dataframe = driver_profits.select('business-month', 'driver_total_pay')
    ...
    output_path = "s3a://" + s3_bucket + "/cw_task2_" + date_time


    # Write the DataFrame to a CSV file
    #new_dataframe.coalesce(1).write.csv(output_path, header = True)
