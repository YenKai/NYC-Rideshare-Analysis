...
if __name__ == "__main__":
    ...
    rideshare_data_df = spark.read.option("header", True).csv("s3a://" + s3_bucket + "/rideshare_data.csv")
    
    taxi_zone_lookup_df = spark.read.option("header", True).csv("s3a://" + s3_bucket + "/taxi_zone_lookup.csv")
    ...
    # Group by 'Month' and 'Pickup_Borough' and count the number of trips
    monthly_counts = df.groupBy('Month', 'Dropoff_Borough').count()
    
    # Rank the pickup boroughs within each month based on trip count
    from pyspark.sql.window import Window
    from pyspark.sql.functions import rank
    
    window_spec = Window.partitionBy('Month').orderBy(col('count').desc())
    ranked_df = monthly_counts.withColumn('rank', rank().over(window_spec))
    
    # Filter the top 5 pickup boroughs for each month
    top_5_boroughs = ranked_df.filter(col('rank') <= 5).orderBy('Month', 'rank')
    
    # Rename columns and select necessary columns for output
    result = top_5_boroughs.select(col('Dropoff_Borough').alias('Dropoff_Borough'),
                                    col('Month'),
